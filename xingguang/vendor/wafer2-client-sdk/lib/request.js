...
function request(options) {
    ...
    function doRequest() {
        ...
        // 增加UA信息
        var uaHeader = {}
        uaHeader[constants.GJ_HEADER_CHANNEL] = config.headerInfo.channel;
        uaHeader[constants.GJ_HEADER_PACKAGEID] = config.headerInfo.packageId;
        ...
        wx.request(utils.extend({}, options, {
            ...
            success: function (response) {
                ...
                if ((data && data.errno === -100) || response.statusCode === 401) {
                    // 检查登录态的接口，不需要自动登录
                    /* if(options.url.indexOf('/user/check') != -1){
